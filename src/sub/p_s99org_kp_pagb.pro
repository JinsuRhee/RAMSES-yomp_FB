PRO p_s99org_kp_pagb, settings
	...
	FOR i=0L, ntime-1L DO BEGIN
		...
		FOR iz=0L, nmetal - 1L DO FOR ie=0L, nelem - 1L DO $
		       cyield_wn(iz,i,ie) = cyield_wn(iz,j,ie) + yield_wn(iz,i,ie) * dt

		FOR iz=0L, nmetal - 1L DO FOR ie=0L, nelem - 1L DO $
		       cyield_sn(iz,i,ie) = cyield_sn(iz,j,ie) + yield_sn(iz,i,ie) * dt

 		FOR iz=0L, nmetal - 1L DO $
	 		cml_wn(iz,i) = cml_wn(iz,j) + ml_wn(iz,i) * dt

 		FOR iz=0L, nmetal - 1L DO $
	 		cml_sn(iz,i) = cml_sn(iz,j) + ml_sn(iz,i) * dt
